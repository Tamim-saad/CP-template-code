...
#define ll long long
#define init ll count=0,zz,t;cin>>t;while(t--)
...
#define printarr(arr,in,n) cout<<ed;for(ll i=in;i<n;i++)cout<<arr[i]<<" ";cout<<ed;
#define print(arr,n) cout<<ed;for(ll i=0;i<n;i++)cout<<arr[i]<<" ";cout<<ed;
...
void build(ll arr[],ll rootPos,ll l,ll r)
{
    ...
    segmantTree[rootPos]=min(segmantTree[rootPos*2],segmantTree[rootPos*2+1]);
    return;
}
...
void update(ll rootPos,ll l,ll r,ll pos,ll newValue)
{
    ...
    if(l==r ) 
    {
        ...
        ll m=(l+r)/2;
        if(l<=pos && pos<=m) update(rootPos*2  ,  l,m,pos,newValue);
        //if(l<=pos && pos<=m)
        else update(rootPos*2+1,m+1,r,pos,newValue); 
...
{
    if(r<L || R<l) if(L>R || l>r || l>R || r<L) return LONG_LONG_MAX;
    if(L<=l && r<=R) return segmantTree[rootPos];
    else
    {
        ll m=(l+r)/2;
...
int main()
{
    ...
    {
      //  count++;
        ll i,j,m,x,y,d,p=0,q=0,n=0,rest=0,k,ans=0,flag=-1,sum=0,bef_even_pos=0,bef_odd_pos=0;
        ll mini=lmax,maxi=-1,dif=0,total=0,same=0,plus=0,minus=0,zero=0;
        ll piv=1,rem=0,fix=0,flex=0,change=0,prev=-1,l=0,r=0,lval,rval,now=0,mid;
